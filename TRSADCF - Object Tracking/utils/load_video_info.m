function [seq,img_files, pos, target_sz, ground_truth, video_path] = load_video_info(base_path, video)
%LOAD_VIDEO_INFO
...
% 	filename = [video_path 'groundtruth_rect' suffix '.txt'];
 	filename = [video_path video '/' 'gt' suffix '.txt'];
     seq.format = 'otb';
	f = fopen(filename);
	...
	video_path = [video_path video '/' 'img/'];
